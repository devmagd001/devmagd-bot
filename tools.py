...
def showFiles(app, SMS, SAVED_MESSAGES, DIRECTORY, username):
    ...
    SMS_DEL = SMS.reply(listfiles, reply_markup=MENU, disable_web_page_preview=True)
    SMS_FILES.append(SMS_DEL)
...
def mostrar_opciones(SMS):
    OPTION = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("🌠 ADD IMAGE", callback_data="tumb"),
                InlineKeyboardButton("📤 AUTO UP.", callback_data="autoup"),
                InlineKeyboardButton("💬 ADD CAPTION", callback_data="caption"),
            ],
            [
                InlineKeyboardButton("📚 FILE FORMAT", callback_data="formatfile"),
                InlineKeyboardButton("🗜 ZIP SIZE", callback_data="zipsize"),
            ],
            [
                InlineKeyboardButton("🎞 VIDEO UPLOAD", callback_data="formatvideo"),
                InlineKeyboardButton("🏷 SAVE DESC", callback_data="savedesc"),
            ],
                    ...
                    Botones.append(
                        [
                            InlineKeyboardButton(
                                f'🔇 {f.split(":")[1]} [{f.split(":")[2]}] ~ {f.split(":")[3]}',
                                callback_data=f"dlvid-{count}",
                            )
                        ]
                    )
                else:
