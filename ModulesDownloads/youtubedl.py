        ...
        if d['status'] == 'downloading':
            ...
            if not self._isTwitch:
                ...
                if d['speed'] is not None:
                    speed = d['speed']
                tiempo = d['_eta_str']
                self.downlad_progres(int(current), int(
                    total), speed, filename, tiempo, self.msg, self.bot)
            else:
                filename = d['filename']
                current = d['downloaded_bytes']
                speed = 0
                if d['speed'] is not None:
                    speed = d['speed']
                tiempo = d['_eta_str']
                self.downlad_progres(int(current), speed,
                                     filename, tiempo, self.msg, self.bot)
            ...
            if 'youtu' in url:
                ...
                try:
                    if 'filesize_approx' in format.keys():
                        SIZE = round(format['filesize_approx'] / 1e+6, 2)
                    elif 'filesize' in format.keys():
                        SIZE = round(format['filesize'] // 1e+6, 2)
                    else:
                        SIZE = 0
                    if SIZE > 1000:
                        SIZE = SIZE // 1000
