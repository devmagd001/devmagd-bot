...
def subir_todo(app, callback_query):
    ...
    stk = SMS.reply_sticker("./assets/subiendo.tgs")
    ...
    for i in FOLDER_FILES:
        if isfile(join(directory, i)):
            count += 1
            uploadOnefile(app, SMS, i, directory, len(FOLDER_FILES), OPTIONS, count)
    SMS.reply_sticker("./assets/fin.webp")
    stk.delete()
...
def borrar_todo(app, callback_query):
    global saved_messages
    username = callback_query.from_user.username
    if "CallbackQuery" in str(callback_query).split(":")[1]:
        SMS = callback_query.message
    else:
        SMS = callback_query
...
def callbackQuery(app, callback_query):
    ...
    if CALLBACK_DATA == "ver":
        ...
        MARKUP = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton("✅ ACTIVAR", callback_data="saveon"),
        ...
        SMS.edit_text(
            "**Los archivos de Telegram se descargarán con el nombre que tenga en su descripción.**",
        ...
        SMS.delete()
        SMS.reply(
            "**📝 Introduzca el nombre para el zip\n🔑 Debajo una contraseña (Opcional)**",
    ...
    # **************************************** ELEGIR FORMATO DE ZIPS ****************************************
        ...
        SMS.edit_text(
            f"**✅ Done, the format will be: `{CALLBACK_DATA}`**",
    ...
    # **************************************** SUBIDA AUTOMATICA ****************************************#
        ...
        MARKUP = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton("📦 COMPRIMIDOS", callback_data="up_compress"),
                    InlineKeyboardButton(
                        "📄 SIN COMPRIMIR", callback_data="up_noCompress"
                    ),
                ],
                [InlineKeyboardButton("☑️ DESACTIVAR", callback_data="desactivate")],
                [InlineKeyboardButton("⏮ REGRESAR ⏮", callback_data="option")],
            ]
        )
        SMS.edit_text(
            "**Cómo desea subir los archivos de forma automática?**",
            reply_markup=MARKUP,
        )
        ...
        SMS.edit_text(
            "📦 **Los archivos se subirán comprimidos**", reply_markup=BUTTON_BACK
        )

    elif CALLBACK_DATA == "up_noCompress":
        OPTIONS["auto_up"] = "up_noCompress"
        SMS.edit_text(
            "📄 **Los archivos se subirán sin comprimir**", reply_markup=BUTTON_BACK
        )

    elif CALLBACK_DATA == "desactivate":
        OPTIONS["auto_up"] = "desactivate"
        SMS.edit_text(" **Subida automática ☑️ DESACTIVADA**", reply_markup=BUTTON_BACK)
    ...
    # **************************************** AÑADIR THUMB ****************************************#
        ...
        SMS.delete()
        SMS.reply(
            "🌠 **Envíame una imagen para mostrar en el archivo cuando se suba a Telegram**",
            reply_markup=ForceReply(),
        )
    ...
    # **************************************** ELEGIR TAMAÑO DE ZIPS ****************************************#
        ...
        SMS.delete()
        SMS.reply("**📚 Introduzca el tamaño de los zips: **", reply_markup=ForceReply())
    ...
    # **************************************** ELEGIR FORMATO DE VIDEO **************************************** #
        ...
        SMS.edit_text(
            "**Eliga cómo se subirán los vídeos a Telegram:**",
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton("🎞 VIDEO", callback_data="Video"),
                        InlineKeyboardButton("📄 DOCUMENTO", callback_data="Documento"),
                    ],
                    [InlineKeyboardButton("⏮ REGRESAR ⏮", callback_data="option")],
                ]
            ),
...
def guardarSticker(app, message):
    sticker = app.download_media(message)
    app.send_document(message.chat.id, sticker, force_document=True)
    unlink(sticker)
...
def Agregar_botones(app, message):
    ...
    message.reply(
        Txt, reply_to_message_id=message.reply_to_message.id, reply_markup=ForceReply()
    )
...
def CarbonAPI(app, message):
    ...
    CODE = (
        message.text.replace("/carbon\n", "")
        .replace("/carbon@rey_testingBot\n", "")
...
@app.on_message(
    filters.reply
    & filters.create(
        lambda f, c, u: u.reply_to_message.text.startswith(
            "📝 Introduzca el nombre del zip"
        )
    )
)
async def subirArchivosComprimidos(app, message):
    ...
    FOLDER_FILES = listdir(directory)
    await uploadFiles(app, message, message.text, directory, FOLDER_FILES, OPTIONS)
...
def Responder_Mensajes(app, message):
    ...
    if SMS_REPLY.text.startswith(
        "Para crear botones, necesitas proporcionar el nombre del botón y la URL del enlace que deseas que el botón lleve. El formato es el siguiente:"
    ):
            ...
            if len(filas) == 1:
                ...
                for i in column:
                    btn.append(
                        InlineKeyboardButton(
                            i.split(" - ")[0],
                            url=i.split(" - ")[1].replace("@", "https://t.me/"),
                        )
                    )
        ...
        try:
            ...
            if zips >= 1 and zips <= 2000:
                ...
                message.reply(
                    f"✅ **Zips establecidos en: [ `{zips} MB` ]**",
                    reply_markup=BUTTON_BACK,
                )
            else:
                message.reply(
                    "❌ **Debes introducir un número entre 1 y 2000**",
                    reply_markup=BUTTON_BACK,
                )
        except ValueError:
            message.reply(
                "❌ **Debes introducir solo números**", reply_markup=BUTTON_BACK
            )
        ...
        compressSelectedFiles(
            ...
            message.text,
            username,
            saved_messages[username]["listCompress"],
            directory,
        )
...
@app.on_message(
    filters.regex("/up_") & ~filters.regex("/up_all") & ~filters.regex("/up_album")
)
def subirArchivo(app, message):
    global saved_messages
    try:
    ...
    try:
        ...
        directory = f"{message.from_user.username}"
    uploadOnefile(app, message, FILE_NAME, directory, 1, OPTIONS, 1)
...
async def subirAlbum(app, message):
    ...
    for i in FOLDER_FILES:
        if fileType(i).isPhoto():
            ...
            ListDocument.append(InputMediaDocument(join(directory, i), thumb=thumb))
            if len(ListDocument) == 10:
                await app.send_media_group(message.chat.id, ListDocument)
                ListDocument = []
    ...
    try:
        ...
        if num.isdigit():
            ...
            if file.endswith(".mp4") or file.endswith(".mkv"):
                sms = await message.reply(f"🖨** Extrayendo Imagenes de: \n`{file}`**")
                ...
                await message.reply("⚠️ El archivo seleccionado no es un video válido")
        else:
            await message.reply(
...
@app.on_message(
    filters.text & filters.create(lambda f, c, u: u.text.startswith("http"))
)
def descargar_enlaces(app, message):
    ...
    try:
        ...
        directory = message.from_user.username
    ...
    if any(keyword in message.text for keyword in keywords):
        CHOSE_FORMAT, saved_messages = download_youtube(
            message, message.text, CHOSE_FORMAT, username, saved_messages
        )
        return
...
def Recibir_Mensajes(app, message):
    ...
    if SMS_TEXT.startswith("/start"):
        ...
        try:
            ...
            return message.reply("**Debe tener un nombre de usuario para usar el Bot**")
        try:
        ...
        if "yadriany" == username:
            ...
            TXT = f"""**
[ ](https://ik.imagekit.io/recuerdos/Logos/File%20Master%20Bot_00000-1.png)
...
Espero que le guste mi servicio y que se sienta satisfecho. 😊
    ...
    if SMS_TEXT.startswith("/cd_"):
        ...
        try:
            ...
            if "/" not in FOLDER:
                if isfile(join(directory, DIC_FILES[username].get(int(FOLDER)))):
                    raise ValueError("El numero indicado no corresponde a una carpeta")
            ...
            if "-" in NUM:
                ...
                if int(INDEX[0]) < int(INDEX[-1]):
                    ...
                    MSG.edit_text(
                        f'{TXT.replace("╔📄 Moviendo archivos...", "╔ ✅ Tarea finalizada")}║\n╚➣ 📂 __{NAME_FOLDER}__',
                ...
                if isdir(FILE):
                    raise ValueError("El numero indicado no corresponde a un archivo")
                else:
                    if "/" in FOLDER:
                        NAME_FOLDER = directory.split("/")[-2]
                        move(FILE, directory.split("/")[-2])
                    ...
                    message.reply(
                        "✅ **Tarea finalizada**\n\n╔ 📄 __{}__\n║\n╚➣ 📂 __{}__".format(
                            DIC_FILES[username].get(int(NUM)), NAME_FOLDER
                        ),
                        reply_markup=BUTTON_FILES,
                    )
        ...
        try:
            ...
            if NUM.isdigit():
                ...
                if int(NUM) > len(listdir(path=directory)):
                    return message.reply(
                        "**⚠️ El número indicado no se encuentra en la lista**",
                        reply_markup=BUTTON_BACK,
                    )
                SMS = message.reply(
                ...
                split(
                    f"./{join(directory, FILE)}", f"./{username}", getBytes("2000.0MiB")
                )
                SMS.edit_text(
                    "**✅ Tarea finalizada con éxito**", reply_markup=BUTTON_FILES
                )
            else:
                message.reply(
                    "**⚠️ Debe introducir el número correspondiente al video\n\nEjemplo: `/split 1`**",
    ...
    elif SMS_TEXT.startswith("/upmf"):
        try:
            NUM = SMS_TEXT.split(" ")[1]
            if NUM.isdigit():
                SMS = message.reply("**🚛 Subiendo a Mediafire...**")
                FILE = DIC_FILES[username].get(int(NUM))
                if int(NUM) > len(listdir(path=directory)):
                    return message.reply(
                        "**⚠️ El número indicado no se encuentra en la lista**",
                ...
                try:
                    var = client.upload_file(f"./{directory}/{FILE}", "mf:/Documents/")
                    # https://www.mediafire.com/file/ts9azf8o4a9mb3a/tuturu-tono-de-notificacion9w8a.mp4/file
                    SMS.edit_text(
                        f"`https://www.mediafire.com/file/{var.quickkey}/{var.filename}/file`",
                        reply_markup=InlineKeyboardMarkup(
                            [
                                [
                                    InlineKeyboardButton(
                                        text="🔗 ENLACE",
                                        url=f"https://www.mediafire.com/file/{var.quickkey}/{var.filename}/file",
                                    )
                                ]
                            ]
                        ),
                ...
                message.reply(
                    "**⚠️ Debe introducir el número correspondiente al archivo\n\nEjemplo: `/upmf 1`**",
                    reply_markup=BUTTON_BACK,
                )
        except Exception as x:
            message.reply(x)
        ...
        if FOLDER == "":
            return message.reply(
                "**Debe escribir el comando, dejar un espacio, el nombre de la carpeta: \nEjemplo: `/mkdir Mi Carpeta`**"
            )
        try:
        ...
        for i in SMS_TEXT.replace("/borrar ", "").split(" "):
            if i.isdigit():
                ...
                if FILE == None:
                    message.reply(f"**⚠️ ERROR: [`{i}`] No se encuentra en la lista**")
                else:
                    Text += f"\n🗑 `{FILE}`"
    ...
    # =============================== DESCOMPRIMIR ARCHIVO SELECCIONADO ===============================#
        ...
        try:
            num = SMS_TEXT.split(" ")[1]
            if len(SMS_TEXT.split(" ")) == 3:
                password = SMS_TEXT.split(" ")[2]
            else:
                password = ""
            ...
            if num.isdigit():
                ...
                if (
                    FILE.endswith("zip") == False
                    and FILE.endswith("rar") == False
                    and FILE.endswith("7z") == False
                ):
                    return message.reply(
                        "**⚠️ El formato del archivo seleccionado no es compatible con la operación solicitada. Por favor, asegúrese de que el archivo tenga la extensión [`.zip`, `.rar`]**",
                        reply_markup=BUTTON_BACK,
                    )
                sms = message.reply(f"♻️ **Descomprimiendo archivo: `{FILE}`**")

                if FILE.endswith("zip"):
                    if password == "":
                        with ZipFile(join(directory, FILE), "r") as zip:
                            zip.extractall(path=join(directory, splitext(FILE)[0]))
                    else:
                        with AESZipFile(join(directory, FILE)) as zip:
                            zip.extractall(
                                path=join(directory, splitext(FILE)[0]),
                                pwd=password.encode(),
                            )
                    ...
                    with SevenZipFile(join(directory, FILE), mode="r") as z:
                        z.extractall(join(directory, splitext(FILE)[0]))
    ...
    elif SMS_TEXT.startswith("https://t.me/") or SMS_TEXT.startswith(
        "https://t.me/joinchat/"
    ):
        if (
            SMS_TEXT.split("/")[2] == "t.me"
            and SMS_TEXT.split("/")[-1].isdigit() == False
            and SMS_TEXT.endswith("?single") == False
        ):
...
async def sendMessage():
    ...
    try:
        await app.send_message(BOT_USER, "**🤖 BOT REINICIADO 🔄**")
    except Exception as x:
        print(x)
...
# Ruta del Usuario
server.router.add_get("/user/{user}", usr_path)
    ...
    while True:
        ...
        async with ClientSession() as session:
            async with session.get(
                f"https://{NAME_APP}.onrender.com/" + "/Despiertate"
            ):
                pass
