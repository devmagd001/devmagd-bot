...
from os import getenv, mkdir, makedirs, unlink, listdir, chdir, getcwd
...
async def uploadFiles(app, message, NM_ZIP, DIRECTORY, FOLDER_FILES, OPTIONS):
    CHAT_ID = message.chat.id
    USERNAME = message.from_user.username
    try:
        makedirs(f"folderZip/{USERNAME}")
    except:
        pass
    try:
        mkdir(f"./storage")
    ...
    if len(NM_ZIP.split("\n")) == 1:
        ...
        NAME, PASSWORD = NM_ZIP.split("\n")
    sms = await message.reply("**📚 Comprimiendo: **")
    NAME_ZIP = f"{NAME}.zip"
    NAME_ZIP = NAME_ZIP.replace(" ", "-")
    # FOLDER_ZIP = join(DIRECTORY, NAME_ZIP)

    if NAME_ZIP.endswith(
        "7z"
    ):  # ==================================================== COMPRIMIR A 7z
        if len(NM_ZIP.split("\n")) == 1:
            filesZip = py7zr.SevenZipFile(NAME_ZIP, "w")
        elif len(NM_ZIP.split("\n")) == 2:
            filesZip = py7zr.SevenZipFile(NAME_ZIP, "w", password=PASSWORD)
    ...
    if (
        round(getsize(NAME_ZIP) / 1000000, 2) > OPTIONS["zip_size"]
    ):  # SI EL ARCHIVO PESA MAS DE 2GB
        await sms.edit_text(f"✂️ **Dividiendo en partes de: {OPTIONS['zip_size']} MB**")
        split(
            f"./{NAME_ZIP}",
            f"./folderZip/{USERNAME}",
            getBytes(f"{OPTIONS['zip_size']}.0MiB"),
        )
        ...
        for f in fileOrd:
            if exists(f"./{USERNAME}-thumb.jpg"):
                thumb = f"./{USERNAME}-thumb.jpg"
            else:
            ...
            try:
                await app.send_document(
                    CHAT_ID,
                    join(f"folderZip/{USERNAME}", f),
                    thumb=thumb,
                    progress=progressupl,
                    progress_args=(sms, len(fileOrd), count, start),
                )
            except:
        ...
        try:
            SIZE = round(getsize(NAME_ZIP) / 1000024, 2)
        except:
            SIZE = "➖"
    ...
    try:
        if len(NM_ZIP.split("\n")) == 2:
            await message.reply(f"**Contraseña: **[ `{PASSWORD}` ]")
    except:
        pass
...
def uploadOnefile(app, message, FILE_NAME, DIRECTORY, TOTAL, OPTIONS, count=0):
    USERNAME = message.from_user.username
    CHAT_ID = message.chat.id
    try:
        makedirs(f"folderZip/{USERNAME}")
    ...
    if (
        round(getsize(FILE_NAME) / 1000000, 2) > OPTIONS["zip_size"]
    ):  # SI EL ARCHIVO PESA MAS DE 2GB
        message.reply(f"✂️ **Dividiendo en partes de: {OPTIONS['zip_size']} MB**")
        split(
            f"./{FILE_NAME}",
            f"./folderZip/{USERNAME}",
            getBytes(f"{OPTIONS['zip_size']}.0MiB"),
        ...
        num = 0
        message.delete()
        pin = message.reply(f"⚜️◤━━━ ☆. **{FILE_NAME.split('/')[-1]}** .☆ ━━━◥⚜️")
        for f in fileOrd:
            if exists(f"./{USERNAME}-thumb.jpg"):
                thumb = f"./{USERNAME}-thumb.jpg"
            else:
                thumb = "./assets/thumb.jpg"
            num += 1
            sms = message.reply(f"**📤 Subiendo: {num}-{len(fileOrd)}**")
            ...
            app.send_document(
                ...
                thumb=thumb,
                progress=progressupl,
                progress_args=(sms, len(fileOrd), num, start),
            )
            unlink(join(f"folderZip/{USERNAME}", f))
            sms.delete()
        app.send_sticker(CHAT_ID, "./assets/fin.webp")
        delpin = pin.pin(both_sides=True)
        delpin.delete()
    else:
        if exists(f"./{USERNAME}-thumb.jpg"):
            thumb = f"./{USERNAME}-thumb.jpg"
            ...
            thumb = "./assets/thumb.jpg"
        sms = message.reply(f"**📤 Subiendo: {count}-{TOTAL}**")
        try:
            SIZE = round(getsize(FILE_NAME) / 1000024, 2)
        except:
            SIZE = "➖"
        ...
        if fileType(file=FILE_NAME).isVideo():
            if OPTIONS["format_video"] == "video":
                ...
                app.send_video(
                    ...
                    FILE_NAME,
                    progress=progressupl,
                    progress_args=(sms, TOTAL, count, start),
                    duration=seconds,
                    thumb=THUMB,
                )
            else:
            ...
            if fileType(file=FILE_NAME).isPhoto():
                try:
                    ...
                    app.send_photo(CHAT_ID, FILE_NAME)
                except:
                    app.send_chat_action(CHAT_ID, enums.ChatAction.UPLOAD_DOCUMENT)
                    app.send_document(
                        CHAT_ID,
...
def compressfiles(app, message, msg, DIRECTORY):
    ...
    if nameZip.endswith(
        ...
        if len(msg.split("\n")) == 1:
            filesZip = py7zr.SevenZipFile(nameZip, "w")
        elif len(msg.split("\n")) == 2:
            filesZip = py7zr.SevenZipFile(nameZip, "w", password=password)
    ...
    if nameZip.endswith(
        ...
        if password == "":
            ...
            pyminizip.compress_multiple(
                files, rutas, join(DIRECTORY, nameZip), password, 9
            )
